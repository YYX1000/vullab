# ruoyi-vue-pro · java17 环境（Docker）

基于 [ruoyi-vue-pro](https://github.com/YunaiV/ruoyi-vue-pro) `master-jdk17` 分支的一键 Docker 环境。

- JDK17 + SpringBoot3 + **SpringSecurity**（jakarta 体系）
- 后端：`YunaiV/ruoyi-vue-pro` 分支 `master-jdk17`（端口 48080）
...
redis    ── 内部（验证码 / 缓存 / 会话）
backend  ── SpringBoot3 后端（内部 48080，不对外暴露）
...
```bash
YUDAO_BRANCH=master-jdk17   # JDK17 + SpringBoot3 + SpringSecurity（默认）
```

> 切换为 JDK8 体系请使用本目录下 `java8` 变体（`master` 分支）。
