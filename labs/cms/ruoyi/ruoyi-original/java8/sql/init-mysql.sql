-- ============================================================
-- RuoYi-Vue v3.9.0 数据库初始化脚本（合并 quartz + ry 主库）
-- 由 MySQL 容器 /docker-entrypoint-initdb.d 自动导入
-- 导入目标库：MYSQL_DATABASE 指定的 ry-vue
-- 默认账号：admin / admin123
-- 注意：本脚本与 .env 中 GIT_BRANCH=v3.9.0 对应，切换版本请同步更换 SQL
-- ============================================================

-- 强制客户端连接使用 utf8mb4：MySQL 容器 entrypoint 导入本脚本时，
-- mysql 客户端默认 charset 可能为 latin1，会把 UTF-8 中文按 latin1 解读
-- 造成中文乱码，此处显式声明避免该问题。
